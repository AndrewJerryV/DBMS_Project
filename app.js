...
// Strip the Netlify function / API prefix so Express routes (defined at root)
// match both local (http://localhost:3000/...) and deployed (/api/...) requests.
app.use((req, res, next) => {
  let url = req.url;
  if (url.startsWith('/.netlify/functions/api')) {
    url = url.slice('/.netlify/functions/api'.length);
